...
export default function ContentWorkspace(props) {
  ...
  return (
    <div className="workbench-content-split">
      <aside className="workbench-content-meta" aria-label="生成准备与结果信息">
        <WorkbenchSection
          code="PREP"
        ...
        >
          <StatusNotice
            ...
            text={isGenerating ? (generationState.statusText || '生成中会自动禁用按钮，避免重复点击；完成后会在右侧更新正文，并在左侧更新反馈和质量检查。') : generationState.statusText}
            className="mb-0"
          />
        ...
        <section className="workbench-meta-block">
        ...
        <section className="workbench-meta-block">
          <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
            ...
            <span className="text-[13px] leading-6 text-[color:var(--muted)]">章节质检</span>
          </div>
          <StatusNotice
        ...
        <section className="workbench-meta-block">
          <div className="flex min-w-0 flex-wrap items-center justify-between gap-3">
            ...
            <span className="text-[13px] leading-6 text-[color:var(--muted)]">剧情线推进</span>
          </div>
          {generationState.storylineProgress || generationState.qualityCheck?.storylineAudit ? (
            ...
            <p className="mt-3 text-[14px] leading-7 text-[color:var(--muted)]">生成完成后，这里会展示本章使用的剧情线、beat 和推进写回状态。</p>
          )}
        </section>
      ...
      <section className="workbench-prose-main" aria-label="正文工作区">
        <WorkbenchSection
          code="PROSE"
          title={generationState.hasContent ? '正文' : '生成结果'}
          description={generationState.hasContent ? '这里只保留正文预览与校改入口。' : '当前还没有可预览的正文内容。'}
